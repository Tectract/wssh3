...
#            print >> sys.stderr, "[received payload of length %d as %s]" % (len(m.data), mode_msg)
        sys.stdout.write(str(m.data,'utf-8'))
...
#            if self is not None:
#                sys.stderr.write("self: "+str(self.keys()))

#            if self.opts.verbosity >= 2:
#                sys.stderr.write('[EOF on stdin, shutting down input]')
        ...
        # support behaviour like: echo foo | wssh3 -l ...
